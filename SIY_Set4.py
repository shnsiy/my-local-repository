...
def bin_to_dec(binary_string):
    ...
    base_10_num = 0
    bin_num = len(binary_string)
    for i in range(bin_num):
        digit = binary_string[bin_num - 1 - i]
        base_10_num += int(digit)*2**i 
...
def telephone_cipher(message):
    ...
    for i in message:
        if i in encoder_dict:
            keypad_num = encoder_dict[i]
            if previous_key and keypad_num[0] == previous_key:
                num_string.append('_')
            num_string.append(keypad_num)
            previous_key = keypad_num[0]
...
def telephone_decipher(telephone_string):
    ...
    string_parts = telephone_string.split('_')
    for part in string_parts:
        i = 0
        while i < len(part):
            for j in range(4, 0, -1):
                if part[i:i+j] in decipher_dict:
                    text_string.append(decipher_dict[part[i:i+j]])
                    i += j
                    break
        else:
            i += 1  
