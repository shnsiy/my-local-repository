...
with open("script (2).json","r") as file:
...
for i,j in jsondata.items():
    jsondata.update({i.upper():j.upper()})
    
for h in jsondata:
    if jsondata[h] in unnecessary_text:
        del jsondata[h]
        
for x in jsondata:
    if x in unnecessary_punctuation:
        del jsondata[x]
...
while j != 2488:
    for i in jsondata.items():
        if len(i) >= 3:
            word = i
            count = jsondata.count(i)
            new_pair = {word:count}
            wordcount_dictionary.update(new_pair)
        if j == 2488:
            break
        j += 1
...
final_wordlist_of_tuples = list(json.items())
...
optimal_F = (344 - (12*optimal_R) - (16*optimal_C)) // 10
optimal_R = (344 - (10*optimal_F) - (16*optimal_C)) // 12
optimal_C = (344 - (10*optimal_F) - (12*optimal_R)) // 16
...
def Collatz_winner(number_list):
    ...
    return max(collatz_length and max_collatz)

print (Collatz_winner(range(1,51)))
